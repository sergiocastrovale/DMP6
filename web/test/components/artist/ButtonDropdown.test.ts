...
describe('artist/ButtonDropdown.vue', () => {
  it('renders exactly one trigger button plus one per option', async () => {
    ...
    const wrapper = await mountSuspended(ButtonDropdown, { props: { label: 'Scan', options } })
  ...
  it('closes on Escape and returns focus to the trigger', async () => {
    const options = makeOptions()
    const wrapper = await mountSuspended(ButtonDropdown, { props: { label: 'Scan', options }, attachTo: document.body })
  ...
  it('does not open when disabled', async () => {
    const options = makeOptions()
    const wrapper = await mountSuspended(ButtonDropdown, { props: { label: 'Scan', options, disabled: true } })
