...
watch(() => terminal.exitCode, (code) => {
  if (code === 0) issuesStore.fetchSummary()
})
...
<template>
  <div class="flex flex-col">
    <IssuesIssueTypeTabs current="overview" />

    <div class="p-6">
      <!-- Header -->
      <div class="mb-6 flex items-center justify-between">
        <div>
          <h1 class="text-xl font-semibold text-white">Metadata Issues</h1>
          <p v-if="issuesStore.summary?.lastAudit" class="mt-0.5 text-sm text-zinc-500">
            Last audit: {{ formatRelative(issuesStore.summary.lastAudit.startedAt) }}
          </p>
          <p v-else-if="!issuesStore.summaryLoading" class="mt-0.5 text-sm text-zinc-500">
