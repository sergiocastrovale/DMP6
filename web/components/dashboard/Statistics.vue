...
<template>
  <div class="flex items-center gap-3 md:gap-5">
    <template v-for="(stat, i) in stats" :key="stat.label">
      <div v-if="i > 0" class="my-1 w-px self-stretch bg-stone-100/15" :class="stat.separatorOnMobile ? '' : 'hidden md:block'" />
      <div class="flex-col gap-0 leading-none" :class="stat.mobileHidden ? 'hidden md:flex' : 'flex'">
        ...
        <div class="mt-1.5 font-mono text-sm uppercase text-stone-100/60">
          {{ stat.label }}
        </div>
      </div>
    </template>
