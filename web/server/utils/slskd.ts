...
export async function relocateDownloadedFiles(args: SlskdMoveArgs): Promise<SlskdMoveResult> {
  ...
  // Normalize everything in the target folder to MP3 (keeps existing mp3s as-is) - gated by
  // Settings.flacToMp3/FLAC_TO_MP3, at Settings.flacToMp3Bitrate/FLAC_TO_MP3_BITRATE kbps. Off
  // leaves FLAC (or whatever the source format was) in place; layout.ts's TRACK_EXTENSIONS handles
