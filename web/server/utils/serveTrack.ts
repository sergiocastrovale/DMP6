...
// Range/ETag streaming + missing-file NAS proxy fallback, shared by /api/audio/[id] and
// /rest/{stream,download}. Callers do their own permission check first (play.view, or the
// Subsonic apiKey auth in server/routes/rest/[...path].ts) - this assumes the caller is allowed.
export const serveTrackFile = async (event: H3Event, id: string, options: ServeTrackOptions = {}) => {
