...
// Batched merge of all (or the given) APPROVED downloads into the library.
export default defineEventHandler(async (event) => {
  ...
  const ids = Array.isArray(body.ids) && body.ids.length
    ? body.ids
    : (await prisma.downloadedRelease.findMany({ where: { status: 'APPROVED' }, select: { id: true } })).map(r => r.id)
