...
ssh_cmd "docker load < /tmp/dmp-image.tar.gz && rm /tmp/dmp-image.tar.gz"
rm "$ARCHIVE"
echo ""
...
scp_cmd "$PROJECT_ROOT/index" "$PROJECT_ROOT/sync" "$PROJECT_ROOT/refresh" "$PROJECT_ROOT/audit" "$PROJECT_ROOT/fix" "$PROJECT_ROOT/nuke" "$PROJECT_ROOT/analysis" "$PROJECT_ROOT/playlists" "$PROJECT_ROOT/delete" "$NAS_USER@$NAS_HOST:$NAS_DEPLOY_PATH/"
ssh_cmd "chmod +x $NAS_DEPLOY_PATH/index $NAS_DEPLOY_PATH/sync $NAS_DEPLOY_PATH/refresh $NAS_DEPLOY_PATH/audit $NAS_DEPLOY_PATH/fix $NAS_DEPLOY_PATH/nuke $NAS_DEPLOY_PATH/analysis $NAS_DEPLOY_PATH/playlists $NAS_DEPLOY_PATH/delete"
