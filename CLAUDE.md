...
- `FavoriteRelease` targets exactly one of `releaseId` (a LocalRelease) or `boxReleaseId` (a dissolved box's MusicBrainzRelease — a box has no LocalRelease, so one favorite for the box, never one per disc); raw-SQL CHECK in `prisma/migrations/20260925000000_favorite_box_release`, Prisma can't express it. Refresh/delete on a dissolved box row fan out over `UnifiedRelease.boxDiscReleaseIds` (`helpers/artistPageLogic.ts` `actionReleaseIds`).
...
### Conventions
- Prisma can't declare btree operator-class indexes: `LocalReleaseTrack.filePath` / `LocalRelease.folderPath` pattern-ops indexes (per-folder `LIKE 'x/%'` lookups) live in raw migration `20260923000100_path_prefix_indexes`, allowlisted in `scripts/test-db`'s drift check. A new raw index needs the same allowlist entry.
- **Settings row** is read only through `web/server/utils/settings.ts` (`getSettingsRow`, 5s TTL, in-flight dedupe, stale-on-error; `SETTINGS_CACHE_TTL_MS=0` in tests). `settingsCache`/`downloadSettings`/`monitorSettings`/`autoScan`/`pauseState`/... only derive DB → env → default from that row; `resolveMusicDir()` is the single library-root lookup (DB wins). Anything that writes `Settings` must call `invalidateSettings()` (the PUT route calls `refreshSettings()`). `server/middleware/00.settingsReady.ts` holds the first requests until the row has loaded (Nitro doesn't await plugins).
- Request bodies: one zod schema per mutating route in `web/server/schemas/`, read with `readBodyOf(event, schema)` (`server/utils/requestValidation.ts`) — a bad or empty body is a 400 naming the field, never a TypeError 500. Schema tests in `test/server/schemas/`.
- Images via `useImageUrl()`. Types in `web/types/`. No scripts logic in web app (Rust in `/scripts`).
- Metadata is source of truth — never filesystem paths/folder names for artist/album/year. One narrow, display-only exception: an ambiguous/unbound release (see the no-guessing rule below) shows its **folder leaf name**, since there is no metadata left to display.
- **No guessing release placement.** Album tag and embedded MB id must be **unanimous** across a folder's tracks (`common::consensus::evaluate`) — never a plurality/majority vote. Untagged tracks are absent evidence, never a veto; a value is "tagged" only after MB-id sanitization (`sanitize_mb_id`). Not unanimous → the release is parked `UNKNOWN` with a human-readable `LocalRelease.statusReason` (verbatim, rendered as-is by the web popover/dialog) instead of a guessed placement, and its MB link/track links are cleared. Exempt: a dissolved box disc (placement comes from the box pass, not tags) and a folded multi-disc survivor (legitimately mixes per-disc tags). Artist ids: `docs/sync_decisions.md` §5. Full detail incl. reason strings/precedence: `docs/no_guessing.md`.
