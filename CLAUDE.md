...
- `FavoriteRelease`/`FavoriteTrack` and `Playlist` where `type = MANUAL` are per-user and private — `userId` set, `@@unique([userId, ...])`, Cascade on user delete; no admin override. `Playlist` where `type` is GENRE/REGION is shared/generated — `userId NULL`, regenerated by `./playlists` from `PlaylistGenerator`, same for every user. A raw-SQL CHECK ties the two together (`(type = 'MANUAL') = (userId IS NOT NULL)`) plus a partial unique index keeps generated slugs globally unique (Prisma's declarative schema can't express either — see `prisma/migrations/20260914000000_user_scoped_favorites_playlists`). Server routes scope via `server/utils/libraryOwnership.ts` (`currentUserId`, `visiblePlaylistsWhere`, `findOwnManualPlaylist`).
- **Plays are per-user, same pattern as favorites.** `LocalReleaseTrackPlay` is the per-`(userId, trackId)` counter (`playCount`/`lastPlayedAt`); release/artist totals are computed at read time via `server/utils/userPlays.ts`, never stored denormalized. `PlayEvent` is the underlying per-listen log (one row per play, `userId`+`trackId`, `listenedSeconds`, `counted`, `skipped`, `source`) — `counted` flips false→true once a listen crosses `shouldScrobble`'s threshold (`helpers/playerLogic.ts`), which is the same moment `LocalReleaseTrackPlay` gets incremented (`server/utils/playEvents.ts`'s `applyPlayEventPatch`, guarded against a double-increment under concurrent PATCHes). Both tables cascade off `User`/`LocalReleaseTrack` delete — no snapshot, no admin override. Client-side tracking (`composables/usePlayEventTracker.ts`) is shared by the normal player and the Explore player, since both route through `stores/player.ts`'s single `playTrack()`; a finish before `counted` is a skip (`helpers/playerLogic.ts`'s `isSkip`) except a page-hide (`sendBeacon` to `/api/play-events/[id]/finish`, since a normal PATCH may not survive unload) and natural end-of-track, neither of which count as a skip. Explore's familiarity score reads recent skips (`recentSkipsByIds`) to downrank tracks skipped in the last 90 days.
...
- **Box set = one MusicBrainzRelease, N `MusicBrainzReleaseMedium` rows**, never several releases (see `docs/sync_decisions.md`). MB has no box-set entity, no id-link disc→standalone album — only shared identity is **recording** (`recordingId`). `index` never folds multi-medium; `dmp_sync::boxset::run_repair` (called from `./tidy`, scoped by artist ids) finds groups two ways — folders sharing a parent, and a root folder plus subfolders all bound to one multi-disc release (`nested_groups`, disc 1 in the album folder / disc 2 below) — and does binding + fold/dissolve + equivalence. Binding: tag consensus first, else `boxset::plan_box_bind` tracklist matcher — folder→medium pairing is a strictest-first ladder in `pair_tracks_at` (exact title 5s → 15s → 60s only once 3+ tracks already paired → containment → qualifier-stripped title → typo; the looser rules need a unique candidate), and a folder keeps the disc the strictest level gave it (`title_rules.rs` holds the shared title rules, also used by `status::check_release_status`). A folder matching **no** disc is left out; an **ambiguous** one rejects the group; ≥2 folders and a majority must resolve. Replay any matcher change first: `boxset::tests::replay_library_dump` (`#[ignore]`d, runs the real matcher over a dump of every sibling group). `mediumCount`/medium rows for releases matched before discs were modelled (2026-09-06) are rebuilt from track disc numbers by tidy (`db::backfill_media_from_track_discs`). A binding the box pass owns (`boxReleaseId` or `mediumPosition` set) wins over the folder's own tags on re-sync, and skips the deluxe-edition upgrade, or the two passes fight and the disc never scores. Equivalence 3 tiers (recording equi-join → artist-scoped title+duration → containment). `mediumCount>1` + ≥2 equivalent media → **dissolves** (each disc binds standalone via `boxReleaseId`/`boxMediumPosition`); 0-1 equivalent → **folds** into one `LocalRelease`. Needs `LocalReleaseMember` row/disc or re-scan re-splits it. Each sibling group is bound/folded/dissolved inside its own error boundary — one group's DB error is logged + counted (`groups_failed`), never aborts the rest of the pass (2026-09-06/09-10 outages: an unhandled error mid-loop silently zeroed out box placement for every group after it, on every run, until fixed — see `docs/sync_decisions.md` §9 "One box never blocks the rest").
...
- `ArtistFact` — "Did you know..." trivia cache, shown on Explore under the mood/era summary while a track plays (`web/components/explore/DidYouKnow.vue`, `GET /api/tracks/[id]/fact`), see `docs/feature_did_you_know.md`. Cascade to a specific `releaseId`/`trackId` when set, or `artistId`-only/general (never demotes to artist-level — a re-indexed/deleted release or track takes its fact with it). Lookup is a specificity cascade: a fact about the exact playing track, else its release, else the artist in general, else the widget doesn't render. Fetched from Genius (`server/utils/genius.ts`) and persisted; once an artist has more than `ARTIST_FACTS_DB_THRESHOLD` (15, `helpers/constants.ts`) stored rows, Genius is skipped entirely and facts are served from the DB only.
...
- **Presence** ("connected now", Settings → Users) is in-memory only, `server/utils/presence.ts` — no DB table, no cross-restart persistence, single-Nitro-instance assumption (same as `server/utils/loginThrottle.ts`). Web tabs heartbeat `POST /me/presence` every 30s + on track change (`composables/usePresenceHeartbeat.ts`, `plugins/presence.client.ts`) and leave via `sendBeacon` on pagehide; a Subsonic client counts too, touched once per `/rest/*` call keyed by its `c` param, with `scrobble`'s `submission=false`/`true` setting/clearing now-playing. An entry drops after `PRESENCE_STALE_MS` untouched; its now-playing track separately falls back to "paused" after the shorter `PRESENCE_PLAYING_STALE_MS` even if the session itself is still online (`helpers/constants.ts`).
