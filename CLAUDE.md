...
Fullscreen WebGL visualizer over playback. Open: player-bar icon, Explore header, or `v` key; `Esc` exits. 4 fragment-shader presets — **chaos, fractal, flow, julia** — switchable in HUD or `1`-`4`/`n`. All registered in `helpers/constants.ts`'s `visualizerPresets` (id/label/description/key); adding/renaming a preset touches only that file + its shader/uniform wiring. `decodeVisualizerPreset` falls back to `chaos` for unknown `localStorage['dmp-visualizer']`. Every preset is a single compiled program/one fullscreen triangle/one draw call — no preset touches a non-default framebuffer or blend state (the old Buddhabrot preset did, via its own multi-pass GPU histogram accumulator; removed for reading as visually weak even after its bugs were fixed, replaced by Flow).

Chaos/Fractal/Julia are all Julia sets, deliberately distinct:
- **Chaos**: `c` searched CPU-side per frame (`helpers/visualizer/juliaPath.ts`) to sit just outside the Mandelbrot boundary (dendrite, not interior blob) — steps along the cardioid's outward normal then bisects an escape-count window (radial cardioid scaling was the original bug — lands inside the period-2 bulb). Camera (zoom/spin) is audio-blind by design (used to jump-scale on kicks — rejected). Colour: CPU hue morph (`helpers/visualizer/hueMorph.ts`, random target every 5-9s) → `uChaosHue` → 3-5 palette anchors (`uChaosPalette`) mixed (not additively layered) by `chaosMix()` in the shared `PRELUDE`, keyed on escape depth/trap proximity. Fractal/Julia/Flow all reuse this same palette state for visual continuity across preset switches.
...
**Flow** (`FLOW` in `helpers/visualizer/shaders.ts`): the one preset not built on Mandelbrot/Julia escape-time maths — a domain-warped fBm plasma, Winamp/MilkDrop-reminiscent, so it doesn't compete with the other three's shared "spiraling dendrite" look. A single fbm layer reads as static mottled clouds; warping the sample coordinates by a second fbm field before a third sample (`p = fbm(p + fbm(p + ...))`) is what makes it read as continuously *flowing* rather than a texture merely panning — 4 octaves, `vnoise()` (value noise on the shared `hash()`) with a fixed per-octave rotation + off-integer scale (`2.02`, not `2.0`) so octaves don't visibly align into a lattice.

Same house rule as Chaos/Fractal/Julia, restated because it's the rule most likely to regress: camera motion (swirl+zoom) is `uTime`-only, never audio-driven. `uMid`/`uTreble` scale the warp's *strength* (turbulence, gently — up to +25%/+15%, not doubling), `uBass` nudges the hot-highlight strength (+/-25% around a 0.5 base) — distinct knobs, none touching position. Colour goes through the identical `chaosMix()`/`chaosAnchor()` pipeline as the other three, keyed on the fbm field value directly with no `pow()` contrast-reshaping and no `fract()` banding (a fract-based band is a sawtooth with a hard seam that flickers hard between two colours as the field drifts past it — the single biggest contributor to an early version reading as fast/strobe-like).

Camera is a spiral swirl (`rot2(uTime*0.02 + 1.1/r)`, stronger near the centre) combined with a perpetual zoom-IN via a 2-sample cross-fade one octave of zoom apart (`uv2 = uv1 * 2.0`, blended by `fract(uTime*0.018)`) — `uv2` at the end of a cycle is bit-for-bit the same coordinate `uv1` starts the next cycle at, so the field is continuous straight through the wrap: an endless slow dive with no pop, sidestepping the float32 precision blowup a real one-way zoom would eventually hit.

`uFlowSeed` (Canvas.vue's `flowSeed`) is a slow continuous orbit (`cos/sin(clock * FLOW_SEED_SPEED) * FLOW_SEED_RADIUS`, ~2600s period) through noise-space — deliberately NOT a hold-then-jump reroll. An earlier version held it still for 20-40s then eased to a fresh random point, which read as "static for a while, then a fast jarring slide to a different scene" — exactly the failure the Buddhabrot-era sampled-region comment already warned about (hold-then-jump reads as a slideshow; continuous sweep reads as alive). A perpetual orbit has no jumps at all, so there's nothing to read as a seizure-inducing flash.
