...
## 18. Round 7 — volume-split compilations sharing one single-medium MB release

Found investigating a user-reported "duplicate editions, all `MISSING_TRACKS`" pattern on Al Jolson's *The Man and the Legend*. Root cause distinct from round 5's scattered tags: here 3 genuinely different physical folders (`Vol. 1`/`Vol. 2`/`Vol. 3`, ~20 non-overlapping tracks each) all embed the **same** `MUSICBRAINZ_ALBUMID` for one generic single-medium 25-track MB release — a budget/public-domain-label tagger stamped one id across several distinct physical products, internally consistent per folder (so round 5's majority-vote fix doesn't catch it — each folder *does* have a real majority for its id, the id is just wrong to share). Sync's Tier 1 correctly follows the (bad) consensus and binds all 3 to the one MB release; each then scores `MISSING_TRACKS` against the full 25-track list since no single folder holds all of it.

Not index fragmentation (round 1's bug) either — these are real, separate, non-1-track folders, not shatter artifacts.

**Scope, library-wide** (`releaseId` shared by >1 `LocalRelease` with different titles): 1,334 groups / 3,447 releases total (2,022 `COMPLETE` coincidentally, 1,049 `MISSING_TRACKS`, 376 `EXTRA_TRACKS`). A tighter heuristic — single-medium MB release, ≥1 member `MISSING_TRACKS`, and the group's local track counts together cover ≥90% of the MB release's own tracklist (i.e. the pieces really do add up to the whole) — narrows this to **506 groups / 1,252 releases**, exported to `docs/specs/spec_tidy_observations_volume_split_candidates.csv`. Read as an educated guess, not certain: misses splits that don't sum close to the full tracklist (partial-coverage or heavily-overlapping splits), and can false-positive where one coincidentally-complete copy sits alongside an unrelated short/broken rip.
