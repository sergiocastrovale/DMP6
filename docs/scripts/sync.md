...
4. **Per release:** skip already-synced (has `releaseId`) unless `--overwrite`/`UNKNOWN` → match Tier 1 (`MUSICBRAINZ_ALBUMID`) → Tier 2 (`MUSICBRAINZ_RELEASEGROUPID`) → Tier 3 (title+artist search, only with no usable id consensus) → else `UNMATCHED`. Majority id must be real consensus (unanimous or strict plurality ≥2; all-distinct = no consensus). **Agreement not just plurality** (`tags_agree_on`): Tier 1 binds by tag only when the id (or its release group) is carried by >half the tagged tracks; a mere plurality is looked up but bound only if its tracklist scores `COMPLETE`, else stays `UNMATCHED` and isn't searched (§7). Tier 2 browses only a release group most tracks agree on. Allow-list: Official Album/EP only (Single binds only when files' own ids point at it; a rejected candidate gets one search-tier retry). Compare track counts → `COMPLETE`/`EXTRA_TRACKS`/`MISSING_TRACKS`/`INCOMPLETE`. Ambiguous (multiple editions, no exact count match) → `UNMATCHED`. Upsert MB release, link tracks; unmatching clears track links.
...
Tiers 1/2 need a **consensus** of embedded ids — accepted only if unanimous or a strict plurality (count≥2, strictly > any rival). All-distinct (each track its original source's id) → no consensus, falls to Tier 3 or `UNMATCHED`.
