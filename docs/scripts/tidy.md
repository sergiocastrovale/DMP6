...
3b. **Disc backfill** (global, pure SQL): `db::backfill_media_from_track_discs` — rebuilds medium rows/`mediumCount` for releases matched before discs were modelled, from stored track disc numbers. Must precede the box pass (keys off `mediumCount>1`). Idempotent.
...
5. **Re-score**: `db::get_rescore_targets` unions 3 sources — scoped `UNKNOWN` releases with a bound release **and no no-guessing `statusReason`** (`docs/no_guessing.md`: that reason is terminal, explicit `AND "statusReason" IS NULL` guard so `--rescore-only --all` can never resurrect one even though `releaseId IS NULL` already excludes it in practice); this run's `touched_local_release_ids`; any scoped release with a track link pointing at a *different* release than the folder is bound to (catches discs a box pass dissolved without the change fully propagating — `apply_dissolve` only sets `UNKNOWN` when something changed, so a stale link can otherwise fall through the cracks). `db::rescore_bound_release` re-runs `check_release_status` against the release on file (no MB call), re-links matched tracks, clears `mbTrackId` on non-matches, writes `matchStatus`. No MB release/no local tracks → `Deferred`, stays `UNKNOWN` for `sync`'s own watermark clause.
...
- **`lastTidiedAt` set while `lastSyncedAt` is NULL is expected**, not a violated invariant — phase 7's identity repair clears `lastSyncedAt` on an artist whose identity it just revoked (so sync re-derives it, `docs/sync_decisions.md` §6), phase 10 then stamps the whole scope including that artist.
...
| `--repair-recording-tags` | Removed — one-off Python script, run once, deleted (`docs/specs/spec_tidy_observations.md` §17) | N/A |
