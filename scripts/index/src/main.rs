...
use clap::Parser;
use common::{
    checkpoint::{clear_index_checkpoint, load_index_checkpoint},
...
use index::deletion::{
    ...
    detect_deleted_folders, dropped_links_line,
};

pub(crate) const AUDIO_EXTENSIONS: &[&str] = &["mp3", "m4a", "opus", "aac", "ogg", "flac"];
...
async fn main() {
    ...
    for (folder_idx, folder_name) in artist_folders.iter().enumerate() {
        let ctx = scan::FolderCtx {
            ...
            use_local,
            use_s3,
            release_img_dir: &release_img_dir,
            artist_img_dir: &artist_img_dir,
            s3_client: &s3_client,
            local_release_members: &local_release_members,
            lookup_memo: &lookup_memo,
            run_hash: &run_hash,
            total_folders,
        ...
        let Some(outcome) = scan::process_folder(folder_idx, folder_name, ctx).await else {
            break;
        };
        total_files += outcome.total_files;
        new_total += outcome.new_total;
        updated_total += outcome.updated_total;
        skipped_total += outcome.skipped_total;
        error_total += outcome.error_total;
        favorites_dropped_total += outcome.favorites_dropped_total;
        playlists_dropped_total += outcome.playlists_dropped_total;
        for (reason, count) in outcome.consensus_reason_totals {
