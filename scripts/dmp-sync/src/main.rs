...
use dmp_common::config::load_config;
use dmp_common::db::create_pool;
use dmp_common::filters::matches_filter;
use dmp_common::lock::{acquire_lock, clear_stale_lock, release_lock};
use dmp_common::progress::sync_progress;
use dmp_common::s3::create_s3_client;
use dmp_common::statistics::update_statistics;
...
async fn main() {
    ...
    if let Err(e) = acquire_lock(&pool, "dmp-sync", pid, &lock_args.to_string()).await {
    ...
    cleanup_ghost_artists(&pool).await.ok();
