...
#[derive(Parser, Debug)]
#[command(name = "dmp-index", about = "Index local music files into the database")]
struct IndexArgs {
...
async fn main() {
    ...
    if let Err(e) = acquire_lock(&pool, "dmp-index", std::process::id(), &args_str).await {
    ...
    if !shutdown.load(Ordering::SeqCst) && !has_filter(&args) {
        let del = detect_deleted_folders(&pool, &scanned_folders).await;
