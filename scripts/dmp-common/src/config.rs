...
impl Config {
    /// Returns music_dir or panics — used by dmp-index which requires it.
...
pub fn load_config(music_dir_override: Option<&str>) -> Config {
    ...
    let project_root = std::env::var("PROJECT_ROOT").unwrap_or_else(|_| {
        std::env::current_dir()
            .ok()
            .and_then(|d| {
                // Walk up from scripts/dmp-index or scripts/dmp-sync to project root
                if d.ends_with("scripts/dmp-index") || d.ends_with("scripts/dmp-sync") || d.ends_with("scripts/sync") {
